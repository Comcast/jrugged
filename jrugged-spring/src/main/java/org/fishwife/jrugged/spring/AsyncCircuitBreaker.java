...
public class AsyncCircuitBreaker extends org.fishwife.jrugged.CircuitBreaker {
	...
	/**
	 * Creates a {@link AsyncCircuitBreaker} with a
	...
	/**
	 * Creates a {@link AsyncCircuitBreaker} with the specified
	...
	/**
	 * Creates a {@link AsyncCircuitBreaker} with a
	 * {@link DefaultFailureInterpreter} and using the supplied
	 * {@link CircuitBreakerExceptionMapper} when client calls are made while the
	 * breaker is tripped.
	 * 
	 * @param name   the name for the {@link AsyncCircuitBreaker}.
	...
	/**
	 * Creates a {@link AsyncCircuitBreaker} with the provided
	...
	/**
	 * Wrap the given service call with the {@link AsyncCircuitBreaker} protection
	 * logic.
	 * 
	 ...
	 * @throws           org.fishwife.jrugged.CircuitBreakerException if the breaker
	 *                   was OPEN or HALF_CLOSED and this attempt wasn't the reset
	 *                   attempt
	 * @throws Exception if the {@link org.fishwife.jrugged.CircuitBreaker} is in
	 *                   OPEN state
	 */
	public <T> ListenableFuture<T> invokeAsync(Callable<ListenableFuture<T>> callable) throws Exception {
		...
		if (!byPass) {
			...
			try {
				...
				// This shouldn't happen because Throwables are handled in the async onFailure
				// callback
				handleFailure(cause);
			}
