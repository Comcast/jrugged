 ...
 * "heartbeat" URL in a web application. The <code>StatusController</code> has a
 * particular {@link MonitoredService} associated with it (a common use case
 * would be to inject a {@link org.fishwife.jrugged.RolledUpMonitoredService}
 * for overall system health. The <code>StatusController</code> writes the
 * current status out in the response body and sets an appropriate HTTP response
 * code. This is useful in a load balancer setting where the load balancer
 * periodically pings a pool of application servers to see if they are "OK" and
 * removes them from the pool if they are not. If the <code>Monitorable</code>
 * is capable of serving requests (i.e. is GREEN or YELLOW) then we return a 2XX
 * response code; otherwise we return a 5XX response code.
 */
