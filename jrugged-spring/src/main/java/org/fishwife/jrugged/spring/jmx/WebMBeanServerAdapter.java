...
/**
 * The WebMBeanServerAdapter provides access to MBeans managed by an
 * {@link MBeanServer} via simple string-based accessor methods. This is
 * particularly useful for implementing a web interface to interact with the
 * MBeans. Names of MBeans and returned values are sanitized using the
 * {@link MBeanStringSanitizer} to make them HTML-friendly.
 *
 * It should be noted that creating a web interface the JMX beans bypasses the
 * JMX security mechanisms that are built into the JVM. If there is a need to
 * limit access to the JMX beans then the web interface will need to be secured.
 */
public class WebMBeanServerAdapter {
	...
	/**
	 * Get the {@link Set} of MBean names from the {@link MBeanServer}. The names
	 * are HTML sanitized.
	 * 
