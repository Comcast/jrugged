 ...
 * web-friendly names.
 *
 * It should be noted that by creating a web interface the JMX beans bypasses
 * the JMX security mechanisms that are built into the JVM. If there is a need
 * to limit access to the JMX beans then the web interface will need to be
 * secured.
 */
public class WebMBeanAdapter {
	 ...
	 * 
	 * @return the {@link Map} of {@link String} attribute names to
	 *         {@link MBeanAttributeInfo} values.
	 */
	 ...
	 * 
	 * @return the {@link Map} of {@link String} operation names to
	 *         {@link MBeanOperationInfo} values.
	 */
	...
	public MBeanOperationInfo getOperationInfo(String operationName)
		...
		throw new OperationNotFoundException(
				"Could not find operation " + operationName + " on MBean " + objectName.getCanonicalName());
	}

	/**
	 * Get all the attribute values for an MBean by name. The values are HTML
	 * escaped.
	 * 
	...
	public Map<String, Object> getAttributeValues()
		...
		AttributeList attributeList = mBeanServer.getAttributes(objectName,
				attributeSet.toArray(new String[attributeSet.size()]));
	 ...
	 * @throws UnsupportedEncodingException if the encoding is not supported.
	 */
	public String getAttributeValue(String attributeName) throws JMException, UnsupportedEncodingException {
	...
	/**
	 * Invoke an operation on an MBean by name. Note that only basic data types are
	 * supported for parameter values.
	 * 
	 * @param operationName the operation name (can be URL-encoded).
