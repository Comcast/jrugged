...
/**
 * This class is a standard Servlet filter that can be configured in web.xml to
 * wrap all request handling in a
 * {@link org.fishwife.jrugged.PerformanceMonitor}. In order to get useful
 * access to the statistics, however, it is most convenient to make use of
 * Spring's {@link org.springframework.web.filter.DelegatingFilterProxy} in
 * <code>web.xml</code> and instantiate this filter within a Spring application
 * context. This will allow the JMX annotations inherited from
 * {@link PerformanceMonitorBean} to take effect, with the result that you can
 * get a high-level performance monitor wrapped around all of your application's
 * request handling.
 */
@ManagedResource
