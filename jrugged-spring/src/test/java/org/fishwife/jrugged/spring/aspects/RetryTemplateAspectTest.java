...
public class RetryTemplateAspectTest {
	...
	public void testRetryWithMissingBean() throws Throwable {
		Mockito.doThrow(new NoSuchBeanDefinitionException("")).when(beanFactory).getBean(TEST_RETRY_TEMPLATE,
	...
	public void testRetry() throws Throwable {
		...
		Mockito.doReturn(template).when(beanFactory).getBean(TEST_RETRY_TEMPLATE,
				org.springframework.retry.support.RetryTemplate.class);
	...
	public void testRetryExceptionWithRecovery() throws Throwable {
		...
		Mockito.doReturn(template).when(beanFactory).getBean(TEST_RETRY_TEMPLATE,
				org.springframework.retry.support.RetryTemplate.class);
		Mockito.doReturn(recoveryCallback).when(beanFactory).getBean(TEST_RETRY_TEMPLATE_RECOVERY,
				RecoveryCallback.class);
	...
	public void testRetryExceptionWithoutRecovery() throws Throwable {
		...
		Mockito.doReturn(template).when(beanFactory).getBean(TEST_RETRY_TEMPLATE,
				org.springframework.retry.support.RetryTemplate.class);
		Mockito.doReturn(null).when(beanFactory).getBean(TEST_RETRY_TEMPLATE_RECOVERY, RecoveryCallback.class);
		Mockito.doThrow(new RuntimeException()).when(mockPjp).proceed();
		try {
			aspect.retry(mockPjp, mockAnnotation);
		} finally {
