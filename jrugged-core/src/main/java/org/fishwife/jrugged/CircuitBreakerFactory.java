...
public class CircuitBreakerFactory {
	 ...
	 * @param config the {@link CircuitBreakerConfig} with the configuration values.
	...
	protected void configureCircuitBreaker(String name, CircuitBreaker circuit, CircuitBreakerConfig config) {
		...
		if (fi instanceof DefaultFailureInterpreter) {
			...
			logger.info("Created CircuitBreaker '{}', resetMillis={}", new Object[] { name, resetMillis });
		}
	}
	...
	private void configureDefaultFailureInterpreter(String name, long resetMillis, CircuitBreaker circuit) {
		...
		logger.info("Created CircuitBreaker '{}', limit={}, windowMillis={}, resetMillis={}",
		...
		if (properties != null) {
			...
			if (propertyOverrideValue != null) {
				try {
					...
					logger.error("Could not parse property override key={}, value={}", key, propertyOverrideValue);
				}
			}
		}
		return null;
