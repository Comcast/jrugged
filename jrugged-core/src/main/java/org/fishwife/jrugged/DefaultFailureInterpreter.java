...
public final class DefaultFailureInterpreter implements FailureInterpreter {
	 ...
	 * 
	 * @param limit        the number of failures that will be tolerated (i.e. the
	 *                     number of failures has to be strictly <em>greater
	 *                     than</em> this number in order to trip the breaker). For
	 *                     example, if the limit is 3, the fourth failure during the
	 *                     window will cause the breaker to trip.
	 ...
	 * If limit is set to <em>n</em> then the <em>(n+1)</em>th failure will trip the
	 * breaker. Mutating the limit at runtime can reset previous failure counts.
	 * 
