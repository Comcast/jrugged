...
public class CircuitBreaker implements MonitoredService, ServiceWrapper {
	...
	/**
	 * The {@link FailureInterpreter} to use to determine whether a given failure
	 * should cause the breaker to trip.
	 */
	protected FailureInterpreter failureInterpreter = new DefaultFailureInterpreter();

	/**
	 * Helper class to allow throwing an application-specific exception rather than
	 * the default {@link CircuitBreakerException}.
	 */
	...
	/**
	 * Creates a {@link CircuitBreaker} with a {@link DefaultFailureInterpreter} and
	 ...
	 * 
	 * @param fi the <code>FailureInterpreter</code> to use when determining whether
	 *           a specific failure ought to cause the breaker to trip
	 */
	 ...
	 * 
	 * @param name the name for the {@link CircuitBreaker}.
	 * @param fi   the <code>FailureInterpreter</code> to use when determining
	...
	/**
	 * Creates a {@link CircuitBreaker} with a {@link DefaultFailureInterpreter} and
	 * using the supplied {@link CircuitBreakerExceptionMapper} when client calls
	 * are made while the breaker is tripped.
	 * 
	 * @param name   the name for the {@link CircuitBreaker}.
	 * @param mapper helper used to translate a {@link CircuitBreakerException} into
	...
	/**
	 * Creates a {@link CircuitBreaker} with the provided {@link FailureInterpreter}
	 * and using the provided {@link CircuitBreakerExceptionMapper} when client
	 ...
	 * @throws CircuitBreakerException if the breaker was OPEN or HALF_CLOSED and
	 *                                 this attempt wasn't the reset attempt
	...
	/**
	 * Causes the {@link CircuitBreaker} to trip and OPEN; no new requests will be
	 * allowed until the <code>CircuitBreaker</code> resets.
	 */
	public void trip() {
	...
	/**
	 * Manually set the breaker to be reset and ready for use. This is only useful
	 * after a manual trip otherwise the breaker will trip automatically again if
	 * the service is still unavailable. Just like a real breaker. WOOT!!!
	 */
	public void reset() {
	 ...
	 * {@link CircuitBreaker}. In this case, it really refers to the status of the
	 * client service. If the <code>CircuitBreaker</code> is CLOSED, we report that
	 * the client is UP; if it is HALF_CLOSED, we report that the client is
	...
	/**
	 * Get the current {@link ServiceStatus} of the {@link CircuitBreaker},
	 * including the name, {@link org.fishwife.jrugged.Status}, and reason.
	 * 
	...
	public ServiceStatus getServiceStatus() {
		...
		switch (state) {
		case OPEN:
			return (canSendProbeRequest ? new ServiceStatus(name, Status.DEGRADED, "Send Probe Request")
	 ...
	 *
	 * @param l number of milliseconds to "cool down" after tripping before allowing
	 *          a "test request" through again
	 */
	...
	/**
	 * Returns a {@link String} representation of the breaker's status; potentially
	 * useful for exposing to monitoring software.
	 * 
	 * @return <code>String</code> which is <code>"GREEN"</code> if the breaker is
	 *         CLOSED; <code>"YELLOW"</code> if the breaker is HALF_CLOSED; and
	 *         <code>"RED"</code> if the breaker is OPEN (tripped).
	 */
	...
	/**
	 * Specifies a set of {@link Throwable} classes that should not be considered
	 * failures by the {@link CircuitBreaker}.
	 * 
	 * @see DefaultFailureInterpreter
	 * @param ignore a {@link java.util.Collection} of {@link Throwable} classes
	 */
	 ...
	 * provides the configuration for determining which exceptions trip the circuit
	 * breaker, in what time interval, etc.
	 *
	 ...
	 *
	 * @param listeners a list of interested parties for {@link CircuitBreaker}
	 *                  status events.
	 */
