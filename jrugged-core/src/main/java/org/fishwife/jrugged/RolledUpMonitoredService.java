...
public class RolledUpMonitoredService implements MonitoredService {
	 ...
	 * @param criticals    the set of {@link MonitoredService} subsystems that are
	 *                     necessary for the system as a whole to be functional
	 * @param noncriticals the set of <code>MonitoredService</code> subsystems that
	 *                     are part of the system as a whole but which are not
	 *                     strictly necessary for functionality.
	 */
	public RolledUpMonitoredService(String name, Collection<? extends MonitoredService> criticals,
	 ...
	 * ServiceStatus is the name of this rolled up service. The {@link Status} is
	 * computed based on the status of the critical and non-critical subsystems. The
	 * {@link List} of reasons contains the reasons reported for each system. Each
	 * service returns the service name in addition to the reason that it has that
	 * status (which is reported by the service itself)
	 *
